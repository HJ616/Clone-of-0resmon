...
async function maybeReopenBasket() {
  if (localStorage.getItem('reopen-basket')) {
    localStorage.removeItem('reopen-basket')
    await openBasket()
  }
}
...
function isPopupUrl(url) {
  try {
    ...
    return (isLoginUrl(url) || isBasketUrl(url))
  } catch (e) {
    return false
  }
}
 ...
 */
async function handleAddToBasketClick(actions, packageId, type = 'single', button = null) {
  ...
  if (button) {
    ...
    button.innerHTML = `
      <svg class="animate-spin h-4 w-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" style="display:inline-block; vertical-align:middle; margin-right:5px;">
...
function updatePackageActionsState(actions, quantity = 0) {
  const
    ...
    removeLink = actions.querySelector('.remove'),
    isZero = quantity === 0
...
async function openMediaLightbox(sourceContainer, index = 0) {
  ...
  // Thumbs container'ı bul - önce .thumbs dene, yoksa .package-media-thumbs dene
  const thumbsSource = sourceContainer.querySelector('.thumbs') ||
    sourceContainer.querySelector('.package-media-thumbs')
  ...
  const thumbsCont = thumbsSource.cloneNode(true)
  const thumbs = Array.from(thumbsCont.querySelectorAll('.thumb-item'))

  if (thumbs.length === 0) {
    console.error('No thumbs found')
    return
  }
  ...
  // Slide'ları bul
  const sourceSlides = sourceContainer.querySelectorAll('swiper-slide')
  ...
  for (const slide of sourceSlides) {
    ...
    // Image için zoom ekle
    const slideImage = clone.querySelector('img')
    if (slideImage && !slideImage.closest('.thumb-item')) {
      slideImage.classList.remove('object-cover')
      slideImage.classList.add('object-contain')
      const parent = slideImage.parentNode
      const zoomableWrapper = document.createElement('div')
      zoomableWrapper.classList.add('swiper-zoom-container')
      parent.insertBefore(zoomableWrapper, slideImage)
      zoomableWrapper.appendChild(slideImage)
    }

    // YouTube iframe'i yeniden yükle
    const youtubeFrame = clone.querySelector('iframe')
    if (youtubeFrame) {
  ...
  const closeButton = document.createElement('button')
  closeButton.classList.value = 'popup-close'
  closeButton.textContent = 'Close' // veya __('Close') kullan
  closeButton.addEventListener('click', () => {
    container.remove()
  ...
  // Thumb click event'leri
  for (const thumb of thumbs) {
    thumb.addEventListener('click', () => {
      if (swiperEl.swiper) {
        const thumbIndex = thumbs.indexOf(thumb)
        swiperEl.swiper.slideTo(thumbIndex, 200)
      }
    })
  }
...
function setPackageActionsListener(actions) {
  ...
  if (addLink && !addLink.dataset.clickSet) {
    ...
    addLink.addEventListener('click', e => {
      e.preventDefault()
      handleAddToBasketClick(actions, packageId, 'single', addLink)
    })
  }
  ...
  if (subscribeLink && !subscribeLink.dataset.clickSet) {
    ...
    subscribeLink.addEventListener('click', e => {
      e.preventDefault()
      handleAddToBasketClick(actions, packageId, 'subscribe', subscribeLink)
    })
  }
  ...
  if (removeLink && !removeLink.dataset.clickSet) {
    removeLink.dataset.clickSet = true
    removeLink.addEventListener('click', async e => {
      e.preventDefault()

      const originalContent = removeLink.innerHTML
      removeLink.classList.add('pointer-events-none', 'opacity-75')
      ...
      actions.classList.add('updating')
      const success = await removeFromBasket(packageId)
      actions.classList.remove('updating')

      removeLink.classList.remove('pointer-events-none', 'opacity-75')
      removeLink.innerHTML = originalContent

      if (success) {
        await onPackageStateChanged(packageId, 0)
      } else {
        newToast(__('Failed to remove from basket'), 'error')
      }
    })
  }
}
    ...
    if (!button.dataset.clickSet) {
      ...
      button.addEventListener('click', async e => {
        ...
        if (packageId) {
          ...
          await removeFromBasket(packageId)
          localStorage.setItem('reopen-basket', '1')
          window.location.href = kInitialUrl
        }
      })
    }
  }
